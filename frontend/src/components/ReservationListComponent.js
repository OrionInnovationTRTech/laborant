...
const ReservationListComponent = () => {
    ...
    const cancelEmail = (labName) => {
        if (window.confirm("Are you sure you want to CANCEL email notification when lab is free?")) {
            axios.put(`${process.env.REACT_APP_SPRING_HOST}/v1/unregisterFromWaitingList`, {}, {
                headers: {
                    'Authorization': 'Basic ' + btoa(localStorage.getItem('username') + ':' + localStorage.getItem('password')),
                },
                params: {
    ...
    const handleRunCommand = (labName) => {
        axios.post(`${process.env.REACT_APP_SPRING_HOST}/v1/labs/command/`, {}, {
            ...
            .catch(error => {
                setModalResponse(prevModalResponse => [...prevModalResponse,
                    <React.Fragment>COMMAND: {modalCommand}<br/>SERVER
                        ERROR: {error.response.data.message}<br/></React.Fragment>]);
            });
    };
    ...
    return (
        <div>
            ...
            <Container>
                                            ...
                                            {expanded && <>
                                                {labs.users === null || labs.users.length === 0 || labs.users.some(user => user.username === loggedInUser) ? (
                                                    labs.reserved ? (labs.reservedBy.username === loggedInUser && (
                                                        <button onClick={() => unreserveLab(labs.labName)}
                                                                className="btn btn-dark align-middle px-1 py-1 mb-1">Unreserve</button>
                                                    )) : (
                                                        console.log("users", labs.users),
                                                            <button onClick={() => handleShowReserveModal(labs.labName)}
                                                                    className="btn btn-dark align-middle px-1 py-1 mb-1">Reserve</button>
                                                    )
                                                ) : null}
                                                {labs.reservedBy !== null && labs.reservedBy.username !== loggedInUser ? (
                                                    labs.mailAwaitingUsers.some(user => user.username === loggedInUser) ? (
                                                        <button onClick={() => cancelEmail(labs.labName)}
                                                                className="btn btn-outline-secondary align-middle px-1 py-1 mb-1"
                                                                style={{color: 'gray'}}>

                                                            Cancel Email
                                                        </button>) : (
                                                        <button onClick={() => sendEmail(labs.labName)}
                                                                className="btn btn-outline-primary align-middle px-1 py-1 mb-1">Get
                                                            Email</button>
                                                    )
                                                ) : null}
                                                <Button onClick={() => handleShowStatusModal(labs.labName)}
                                                        className="btn btn-success d-md-flex align-middle justify-content-center px-2 py-1 mb-1">Run
                                                    Status</Button>
                                                <Button onClick={() => handleOpenModal(labs.labName)}
                                                        className="align-middle justify-content-center px-1 py-1 mb-1">Run
                                                    Command</Button>
                                            </>}
                                        </div>
                                    </td>
                ...
                <RunStatusModal labName={modalLabName} showModal={showStatusModal}
                                handleCloseModal={handleCloseStatusModal}/>
                <LabReservationForm labName={modalLabName} showModal={showReserveModal}
                                    handleCloseModal={handleCloseReserveModal}/>
