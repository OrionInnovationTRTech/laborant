...
public class LabService {
    ...
    public void checkLabs() {
        ...
        if(userRepo.findAll().isEmpty()){
            log.info("[checkLabs] no user found in database");
            User user = new User();
            user.setUser_role("ADMIN");
            user.setUsername("admin");
            user.setPassword(PasswordConfig.passwordEncoder().encode(defaultUserPassword));
            user.setEmail("laborant.orion@gmail.com");
            userRepo.save(user);
            asyncService.sendCredentialsWithEmail(user.getEmail(), user.getUsername(), defaultUserPassword);
        }
