...
public class AdminService {
    ...
    public String addNewLab(String labName, String username, String password, String host, Integer port) throws IOException {
        ...
        if (Objects.isNull(searchLab)) {
            ...
            if (Objects.isNull(searchHostUserPair)) {
                ...
                if (isLabReachable(labToBeAdded.getHost())) {
    ...
    public Lab addNewLab(Lab lab) {
        ...
        if (Objects.isNull(searchLab)) {
            ...
            if (Objects.isNull(searchHostUserPair)) {
                if (isLabReachable(lab.getLabName())) {
                    lab = labRepo.save(lab);
                    log.info("[addNewLab] adding new lab named: {}", lab.getLabName());
                    return lab;
                } else {
                    log.error("[addNewLab] lab is not reachable");
                    throw new NotConnected("The lab is not reachable");
                }
            } else {
    ...
    public boolean isLabReachable (String labName){
        log.info("[isLabReachable] called");
        if (findLabByName(labName) == null) {
            return false;
        }
            try {
                if (InetAddress.getByName(labRepo.findByLabName(labName).getHost()).isReachable(4000)){
